...
export default function LoginPage() {
  ...
  const validate = () => {
    ...
    else if (!/\S+@\S+\.\S+/.test(email)) newErrors.email = ERROR_MESSAGES.email_invalid;

    if (!password) newErrors.password = ERROR_MESSAGES.password_required;
    else if (password.length < 6) newErrors.password = ERROR_MESSAGES.password_short;
  ...
  const mapBackendError = (error: any) => {
    ...
    if (backendMessage.includes('not found')) return ERROR_MESSAGES.user_not_found;
    if (backendMessage.includes('disabled')) return ERROR_MESSAGES.account_disabled;
    if (backendMessage.includes('too many')) return ERROR_MESSAGES.too_many_attempts;
